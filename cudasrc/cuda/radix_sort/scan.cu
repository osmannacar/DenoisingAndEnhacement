...
{
    // Zero out d_out
    GPU_HANDLE_ERROR(cudaMemset(d_out, 0, numElems * sizeof(unsigned int)));
    ...
    GPU_HANDLE_ERROR(cudaMalloc(&d_block_sums, sizeof(unsigned int) * grid_sz));
    GPU_HANDLE_ERROR(cudaMemset(d_block_sums, 0, sizeof(unsigned int) * grid_sz));
    ...
    if (grid_sz <= max_elems_per_block)
    {
        ...
        GPU_HANDLE_ERROR(cudaMalloc(&d_dummy_blocks_sums, sizeof(unsigned int)));
        GPU_HANDLE_ERROR(cudaMemset(d_dummy_blocks_sums, 0, sizeof(unsigned int)));
        ...
        gpu_prescan<<<1, block_sz, sizeof(unsigned int) * shmem_sz>>>(d_block_sums, 
                                                                    ...
                                                                    shmem_sz,
                                                                    max_elems_per_block);
        GPU_HANDLE_ERROR(cudaFree(d_dummy_blocks_sums));
    }
    ...
    else
    {
        ...
        GPU_HANDLE_ERROR(cudaMalloc(&d_in_block_sums, sizeof(unsigned int) * grid_sz));
        GPU_HANDLE_ERROR(cudaMemcpy(d_in_block_sums, d_block_sums, sizeof(unsigned int) * grid_sz, cudaMemcpyDeviceToDevice));
        sum_scan_blelloch(d_block_sums, d_in_block_sums, grid_sz);
        GPU_HANDLE_ERROR(cudaFree(d_in_block_sums));
    }
    
    //// Uncomment to examine block sums
    //unsigned int* h_block_sums = new unsigned int[grid_sz];
    //GPU_HANDLE_ERROR(cudaMemcpy(h_block_sums, d_block_sums, sizeof(unsigned int) * grid_sz, cudaMemcpyDeviceToHost));
    //std::cout << "Block sums: ";
