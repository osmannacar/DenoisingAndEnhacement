...
void radix_sort(unsigned int* const d_in, unsigned int* const d_out, unsigned int d_in_len)
{
    ...
    GPU_HANDLE_ERROR(cudaMalloc(&d_prefix_sums, sizeof(unsigned int) * d_prefix_sums_len));
    GPU_HANDLE_ERROR(cudaMemset(d_prefix_sums, 0, sizeof(unsigned int) * d_prefix_sums_len));

    unsigned int* d_block_sums;
    unsigned int d_block_sums_len = 4 * grid_sz; // 4-way split
    GPU_HANDLE_ERROR(cudaMalloc(&d_block_sums, sizeof(unsigned int) * d_block_sums_len));
    GPU_HANDLE_ERROR(cudaMemset(d_block_sums, 0, sizeof(unsigned int) * d_block_sums_len));

    unsigned int* d_scan_block_sums;
    GPU_HANDLE_ERROR(cudaMalloc(&d_scan_block_sums, sizeof(unsigned int) * d_block_sums_len));
    GPU_HANDLE_ERROR(cudaMemset(d_scan_block_sums, 0, sizeof(unsigned int) * d_block_sums_len));
    ...
    for (unsigned int shift_width = 0; shift_width <= 30; shift_width += 2)
    {
        ...
        //GPU_HANDLE_ERROR(cudaMemcpy(h_test, d_in, sizeof(unsigned int) * d_in_len, cudaMemcpyDeviceToHost));
    ...
    GPU_HANDLE_ERROR(cudaMemcpy(d_out, d_in, sizeof(unsigned int) * d_in_len, cudaMemcpyDeviceToDevice));

    GPU_HANDLE_ERROR(cudaFree(d_scan_block_sums));
    GPU_HANDLE_ERROR(cudaFree(d_block_sums));
    GPU_HANDLE_ERROR(cudaFree(d_prefix_sums));
}
